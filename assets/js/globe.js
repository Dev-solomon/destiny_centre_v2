...
const scriptureData = {
  ...
  europe: [
    ...
    {
      ref: "Luke 19:10",
      passage: "For the Son of Man came to seek and to save the lost.",
    },
    {
      ref: "Psalm 96:3",
      passage:
...
function createBadges(continent) {
  ...
  scriptures.forEach((scripture, index) => {
    ...
    badge.className = `scripture-badge ${
      badgePositions[index % badgePositions.length]
    }`;
    ...
    badge.addEventListener("mouseenter", (e) =>
      showTooltip(e, scripture.passage)
    );
...
continentLinks.forEach((link) => {
  link.addEventListener("click", (e) => {
    ...
    if (currentContinent === continent) {
      // Hide if clicking same continent
      hideBadges();
      currentContinent = null;
    } else {
      // Show new continent scriptures
      hideBadges();
      setTimeout(() => {
